...
class SimpleCIL(BaseModel):
    ...
    def __init__(self, task_loader, result_logger, config, checkpoint_path, dataset, model_name, model_path, local_ce, seed, device):
        super(SimpleCIL, self).__init__(task_loader, result_logger, config, checkpoint_path, dataset, model_name, local_ce, seed, device)
    ...
    def fit(self, iter):
        ...
        class_src, class_dst, text_dataset_iso, text_dataset_joint, train_loader, valid_loader, _, _ = self.task_loader.get_task(0)
        ...
        for epoch in range(self.config['epochs']):
            loss = self.train(0, epoch, self.model, text_dataset_iso, train_loader, optimizer, class_src, class_dst, self.config, self.device)
            progress_bar.write("Session: {} | Epoch: {} | Loss: {:.4f}".format(0, epoch, loss))

            if epoch > 0 and epoch % self.config['valid_epoch'] == 0:
                acc_valid, f1_valid = self.valid(self.model, text_dataset_joint, valid_loader, class_src, class_dst, self.config, self.device)
