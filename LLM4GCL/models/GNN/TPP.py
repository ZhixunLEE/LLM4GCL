...
class TPP(BareGNN):
    ...
    def __init__(self, task_loader, result_logger, config, checkpoint_path, dataset, local_ce, model_name, seed, device):
        super(TPP, self).__init__(task_loader, result_logger, config, checkpoint_path, dataset, local_ce, model_name, seed, device)
        ...
        if self.task_loader.base_session is not None:
            cls_head_base = LogReg(self.hidden_dim, self.task_loader.base_session).cuda()
            cls_head_incre = LogReg(self.hidden_dim, self.task_loader.ways).cuda()
            # cls_head = LogReg(self.input_dim, self.task_loader.sessions).cuda()
            self.classifiers = ModuleList([copy.deepcopy(cls_head_base)])
            for _ in range(self.session_num - 1):
                self.classifiers.append(cls_head_incre)
        else:
            # cls_head = LogReg(self.hidden_dim, self.task_loader.ways).cuda()
            cls_head = LogReg(self.input_dim, self.task_loader.ways).cuda()
    ...
    def train(self, curr_session, curr_epoch, model, text_dataset, train_loader, optimizer, class_offset, config, device):
        ...
        for _, batch in enumerate(train_loader):
            ...
            embeds = data.x[subset][mapping].to(device)
    ...
    def fit(self, iter):
        ...
        train_idx_joint, valid_idx_joint, test_idx_joint = [], [], []
        for curr_session in range(self.session_num):
            class_src, _, text_dataset_iso, text_dataset_joint, train_loader, valid_loader, test_loader_isolate, test_loader_joint = self.task_loader.get_task(curr_session)
            optimizer = optimizers[curr_session]

            train_idx = self.task_loader.train_idx_per_task[curr_session]
            train_idx_joint.extend(train_idx)
            ...
            for epoch in range(self.config['epochs']):
                loss = self.train(curr_session, epoch, self.model, text_dataset_iso, train_loader, optimizer, class_src, self.config, self.device)
                progress_bar.write("Session: {} | Epoch: {} | Loss: {:.4f}".format(curr_session, epoch, loss))

                if epoch > 0 and epoch % self.config['valid_epoch'] == 0:
                    acc_valid, f1_valid = self.valid(curr_session, self.model, text_dataset_iso, valid_loader, class_src, self.config, self.device)
            ...
            test_idx_joint.extend(test_idx)
            ...
            subset_joint, edge_index_joint, mapping_joint, _ = k_hop_subgraph(test_idx_joint, self.config['layer_num'], text_dataset_joint.data.edge_index, relabel_nodes=True)
            subgraph_joint = Data(x=text_dataset_joint.data.x[subset_joint], y=text_dataset_joint.data.y[subset_joint], edge_index=edge_index_joint)

            taskid = self.gettaskid(curr_session, prototypes, subgraph, mapping)
            taskid_joint = self.gettaskid(curr_session, prototypes, subgraph_joint, mapping_joint)

            curr_acc_test_isolate, curr_f1_test_isolate = self.evaluate(taskid, self.model, text_dataset_iso, test_loader_isolate, class_src, self.config, self.device)
            curr_acc_test_joint, curr_f1_test_joint = self.evaluate(taskid_joint, self.model, text_dataset_joint, test_loader_joint, class_src, self.config, self.device)
            
            acc_list = []
            for s in range(curr_session):
                class_src, _, text_dataset_iso, _, _, _, test_loader_isolate, _ = self.task_loader.get_task(s)
                ...
                prev_acc_test_isolate, prev_f1_test_isolate = self.evaluate(taskid, self.model, text_dataset_iso, test_loader_isolate, class_src, self.config, self.device)
